    ...
    <div class="card-body">
      ...
      <details class="card-details">
        ...
        <ul class="ingredient-list">
          ${scaled.map(ing => `<li><span class="ing-qty">${ing.qty}${ing.unit && ing.unit !== 'pcs' ? ing.unit : ing.unit === 'pcs' ? '' : ''}</span> ${ing.name}</li>`).join('')}
        </ul>
      </details>
      <details class="card-details">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Diet preference banner if not set
  const diet = getSavedDiet();
  const banner = document.getElementById('diet-banner');
  if (banner) {
    if (!diet) banner.style.display = 'flex';
    else banner.style.display = 'none';
  }
});

// Export for modules (unused but here for potential future use)
window.PreperApp = {
